...
export function NotificationsDropdown() {
  ...
  const getIcon = (type: Notification['type']) => {
    switch (type) {
      case 'comment_mention': return <MessageSquare className="h-3.5 w-3.5 text-primary" />;
      case 'access_granted': return <UserPlus className="h-3.5 w-3.5 text-success" />;
      case 'comment_reply': return <Reply className="h-3.5 w-3.5 text-warning" />;
    }
  };
            ...
            notifications.map(n => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`leading-snug ${!n.is_read ? 'font-medium' : 'text-muted-foreground'}`}>
                    {n.message}
                  </p>
