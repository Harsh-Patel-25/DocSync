    ...
    if (atMatch) {
      ...
      if (data) {
        ...
        const filtered = (profiles || []).filter(
          p => p.display_name.toLowerCase().includes(q) || (p.email && p.email.toLowerCase().includes(q))
        );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {topLevel.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">No comments yet. Use @ to mention collaborators.</p>
        )}
